...
public class Board extends JComponent{
	 ...
	 * @modifies columnNo
	 */
	public Board( int row, int column,InfoBoard ib){
	...
	public Board(String s, int row, int column,InfoBoard ib){
		rowNo = row;
		columnNo = column;
		board = new GameObjects[row][column];
	...
	public BasicLokum createRandomLokum(int tileNum,int row,int col){
		BasicLokum lok = null;
		lok = new BasicLokum(tileNum + 1,row,col);
		return lok;
	}
	/**
	 * Put obstacles accordingly level's properties.
	 * 
	 * @requires 	game must be started.
	 * @requires	board must be created.
	 * @requires	level must contain obstacle..
	 * @modifies 	put obstacles on the board.
	 */
